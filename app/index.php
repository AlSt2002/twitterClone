...
<?php include "inc/header.php" ?>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function retweet() {
      ...
      var url = 'Retweets.php?tweetID=' + tweetID;

      var p = this.nextElementSibling;
      var my_retweetSvg = this.firstElementChild;
      ...
      xhr.onreadystatechange = function () {
        if(xhr.readyState == 4) {
          if(xhr.status == 200) {
              var json = JSON.parse(xhr.responseText);
              if(json[0].result == "true") {
                var numretweets = parseInt(p.innerHTML) + 1;
                p.innerHTML = numretweets;
                my_retweetSvg.style.fill = 'green';

              }
              else {
                var numretweets = parseInt(p.innerHTML) - 1;
                p.innerHTML = numretweets;
                my_retweetSvg.style.fill = 'black';
              }
          }
        }
      }
    ...
    function isRetweeted(user_id, tweet_id) {
        return new Promise((resolve, reject) => {
          var  url = `CheckRetweets.php?user_id=${user_id}&tweet_id=${tweet_id}`;
          var xhr = new XMLHttpRequest();
          xhr.open('GET', url, true);
          xhr.onreadystatechange = function() {
            if(xhr.readyState == 4) {
    ...
    function bindbuttons() {
        ...
        var likeButtons = document.querySelectorAll('.likeButton');
          ...
          for(let i = 0; i < retweetButtons.length; i++) {
            console.log("Retweet binded");
            retweetButtons.item(i).addEventListener('click' , retweet);
            console.log(retweetButtons.item(i));
          }
    }
    ...
    window.onload = function() {
        ...
        setInterval(getNumberOfUnreadNotifications, 4000);
